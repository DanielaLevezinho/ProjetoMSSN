...
public class Eye {
	...
	public Eye(Boid me, List<Body> allTrackingBodies){
		this.me = me;
		findNewTarget(allTrackingBodies);
	}
	...
	public void findNewTarget(List<Body> allTrackingBodies) {
		this.allTrackingBodies = allTrackingBodies;

		int maxRandom = allTrackingBodies.size();
		if (maxRandom < 1) {
			return;
		}
		this.random = randomGenerator.nextInt(maxRandom);
		target = allTrackingBodies.get(random);
	}
